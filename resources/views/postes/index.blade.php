                  ...
                  <tbody class="bg-white">
                    @foreach($postes as $poste)
                      <tr class="whitespace-nowrap">
                        ...
                        <x-link-button onclick="event.preventDefault(); document.getElementById('destroy{{ $poste->id }}').submit();">
                            @lang('Delete')
                        </x-link-button>
                        <form id="destroy{{ $poste->id }}" action="{{ route('postes.destroy',$poste->id) }}" method="POST" style="display: none;">
                            @csrf
                            @method('DELETE')
                        </form>
                      </tr>
